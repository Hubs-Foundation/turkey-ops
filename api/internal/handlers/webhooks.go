...
var GhaTurkey = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	// if err != nil {
	// 	internal.Logger.Error("publishToNamespaceTag failed: " + err.Error())
	// }
	http.Error(w, http.StatusText(http.StatusNotFound), http.StatusNotFound)
})
...
var Dockerhub = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	internal.Logger.Debug("channel: " + channel + ", fulltag: " + fulltag)
...
// 		nsName := item.Name
// 		internal.Logger.Debug("nsName: " + nsName)
...
// 					internal.Logger.Debug("c.Image: " + c.Image + ", repoName: " + repoName)
